...
_(e.g. Use the `queue-cleardown.yml` playbook to safely cleardown the processing queue nightly)_
...
_(e.g. CloudInit bootstraps the installation of Ansible - Ansible then drives all system and application level configuration except for the XYZ service which is configured via `App.config` files in Git)_
